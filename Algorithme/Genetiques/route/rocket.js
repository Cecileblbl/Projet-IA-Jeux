...
class Rocket {
  ...
  calcFitness() {
    ...
    if (this.outofRoad) this.fitness *= 0.1; // lose 90% of fitness being out of road
  ...
  update() {
    ...
    if (!this.isOnRoad(road)) {
      // If the car is not on the road, stop the car
      this.vel.mult(0);
    }
  }
  ...
  isOnRoad(road) {
    ...
    for (let i = 0; i < road.segments.length; i++) {
      let current = road.segments[i];
      let next = road.segments[(i + 1) % road.segments.length];

      // Create a polygon representing the road segment
      let polygon = [
        [current.left.x, current.left.y],
        [next.left.x, next.left.y],
        [next.right.x, next.right.y],
        [current.right.x, current.right.y],
      ];

      // Check if the rocket is inside the polygon
      // Check if the rocket is inside the polygon
      if (this.pointInPolygon(this.position.x, this.position.y, polygon)) {
        this.outofRoad = true;
        return true;
      }
    }

    // If the rocket is not inside any road segment, it is not on the road
    this.outofRoad = false;
    return false;
  }
}
