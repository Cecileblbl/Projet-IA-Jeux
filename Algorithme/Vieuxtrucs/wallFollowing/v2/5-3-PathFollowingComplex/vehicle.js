...
class Vehicle {
  ...
  applyBehaviors(vehicles, path) {
    ...
    if (this.couleur !== "green") {
      ...
      let forceSuiviChemin = this.followWall(path);
  ...
  followWallb(path) {
    ...
    if (worldRecord > path.radius) {
      return this.seek(target);
    } else if (worldRecord < path.radius);
    {
      // on est trop près du mur, on va se repousser
      return this.repulsion(target);
    }
  }
  ...
  followWallc(path) {
    ...
    // On cherche le point du chemin le plus proche
    let closestPoint = null;
    let worldRecord = Infinity; // Initialisation à l'infini pour trouver le plus proche
    ...
    for (let i = 0; i < path.points.length; i++) {
      ...
      // Calcul de la distance entre le point prédit et le point projeté sur le segment
      let d = p5.Vector.dist(predictpos, normalPoint);

      // Mise à jour du point le plus proche
      if (d < worldRecord) {
        worldRecord = d;
    ...
    // Trouver le vecteur tangent au chemin au point le plus proche
    let tangent = p5.Vector.sub(closestPoint, this.position);
    tangent.normalize();

    // Trouver un point sur le chemin à une distance 'd' du point le plus proche
    let target;
    if (worldRecord > path.radius + d) {
      target = p5.Vector.add(closestPoint, p5.Vector.mult(tangent, d));
    } else {
      target = closestPoint.copy(); // Suivre le point le plus proche du chemin
    }

    // Dessiner des informations de débogage
    if (debug) {
      // future position devant le vaisseau
      stroke(0);
      fill(0);
  ...
  followWall(path) {
    ...
    if (onWall === true) {
      // parcourir le chemin en restant a une distance du mur
      let predict = this.velocity.copy();
      predict.normalize();
      predict.mult(25);
      let predictpos = p5.Vector.add(this.position, predict);
  ...
  update() {
    ...
    if (this.couleur == "green") {
      ...
      if (
        this.cheminDerriereWanderer.length > this.longueurMaxiCheminDerriere
      ) {
  ...
  repulsion(target) {
    let desired = p5.Vector.sub(this.position, target); // A vector pointing from the position to the target
