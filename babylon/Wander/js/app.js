...
function startGame() {
  ...
  // Ajout des véhicules
  addVehicles(15); // Ajoute 15 véhicules à la scène

  engine.runRenderLoop(() => {
    vehicles.forEach(vehicle => {
      vehicle.wander(); // Applique le comportement d'errance
      vehicle.update(); // Met à jour la position en fonction de la vitesse et de la direction
    });
    scene.render();
  });
...
// Ajout d'un effet de post-processing
  ...
  shadowGenerator.addShadowCaster(sphere); // sphere projete une ombre
...
class Vehicle {
  constructor(scene, position) {
    this.scene = scene;
    // Initialise la position du véhicule
    this.position = new BABYLON.Vector3(position.x, position.y, position.z);
    // Définis une vélocité initiale aléatoire
    ...
    diameterTop: 0, diameterBottom: 4, height: 4, tessellation: 4}, this.scene);

    this.mesh.position = this.position;
    // Oriente la pyramide pour qu'elle ressemble à une flèche
    ...
    this.wanderTargetMesh = BABYLON.MeshBuilder.CreateSphere("wanderTarget", { diameter: 0.5 }, this.scene);
  ...
  wander() {
    let change = 0.4; // Variation maximale de l'angle
    ...
    // Mise à jour des points de la ligne
    ...
    let directionToTarget = wanderTargetPosition.subtract(this.position).normalize();
